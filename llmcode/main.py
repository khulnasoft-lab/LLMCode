...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run llmcode in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track llmcode's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(".llmcode"):
            patterns_to_add.append(".llmcode*")
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".llmcode.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading llmcode model settings: {e}")
        return 1
...
def load_dotenv_files(git_root, dotenv_fname, encoding="utf-8"):
    ...
    oauth_keys_file = Path.home() / ".llmcode" / "oauth-keys.env"
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    ...
    model_metadata_files += generate_search_path_list(
        ".llmcode.model.metadata.json", git_root, model_metadata_fname
    )
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fname = Path(".llmcode.conf.yml")
    ...
    if args.analytics is not False:
        if analytics.need_to_ask(args.analytics):
            ...
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve llmcode?"
            )
    ...
    if args.llmcodeignore:
        ignores.append(args.llmcodeignore)
    ...
    if args.apply:
        ...
        # For testing #2879
        # from llmcode.coders.base_coder import all_fences
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = Path.home() / ".llmcode" / "installs.json"
