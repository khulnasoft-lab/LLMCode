...
from llmcode import __version__, models, prompts, urls, utils
...
from llmcode.io import ConfirmGroup, InputOutput
from llmcode.linter import Linter
from llmcode.llm import litellm
from llmcode.models import RETRY_TIMEOUT
from llmcode.reasoning_tags import (
    REASONING_TAG,
...
from llmcode.utils import format_content, format_messages, format_tokens, is_image_file
from llmcode.waiting import WaitingSpinner
...
class Coder:
    ...
    repo = None
    last_llmcode_commit_hash = None
    llmcode_edited_files = None
        ...
        **kwargs,
        ...
        if from_coder:
            ...
            update = dict(
                ...
                llmcode_commit_hashes=from_coder.llmcode_commit_hashes,
    ...
    def get_announcements(self):
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .llmcodeignore"
                )
    ...
    def __init__(
        ...
        self.llmcode_commit_hashes = set()
        ...
        if llmcode_commit_hashes:
            self.llmcode_commit_hashes = llmcode_commit_hashes
        else:
            self.llmcode_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(
                    f"Skipping {fname} that matches llmcodeignore spec."
                )
                continue
    ...
    def warm_cache(self, chunks):
        ...
        delay = 5 * 60 - 5
        delay = float(os.environ.get("LLMCODE_CACHE_KEEPALIVE_DELAY", delay))
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.llmcode_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(
                fnames=edited, context=context, llmcode_edits=True, coder=self
            )
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_llmcode_commit_hash = commit_hash
        self.llmcode_commit_hashes.add(commit_hash)
        self.last_llmcode_commit_message = commit_message
        if self.show_diffs:
