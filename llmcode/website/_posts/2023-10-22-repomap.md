...
excerpt: Tree-sitter allows llmcode to build a repo map that better summarizes large code bases.
...
To address these issues, llmcode
...
*To code with GPT-4 using the techniques discussed here, just install [llmcode](https://llm.khulnasoft.com/docs/install.html).*
...
Llmcode also strives to reduce the manual work involved in
coding with AI.
...
Llmcode sends a **repo map** to GPT along with
...
Mapping out the repo like this provides some key benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at in more detail. GPT will then ask to see these specific files, and llmcode will automatically add them to the chat context.
...
Llmcode uses all of these definitions and references to
...
[ctags based map](https://llm.khulnasoft.com/docs/ctags.html)
...
- Thanks to `py-tree-sitter-languages`, we get full support for many programming languages via a python package that's automatically installed as part of the normal `python -m pip install -U llmcode`.
...
- Tree-sitter integration is a key enabler for future work and capabilities for llmcode.
...
Right now, llmcode relies on the user to specify which source files
will need to be modified to complete their request.
Users manually "add files to the chat" using llmcode's `/add` command,
...
just install [llmcode](https://llm.khulnasoft.com/docs/install.html).

## Credits

Llmcode uses
[modified versions of the tags.scm files](https://github.com/khulnasoft-lab/llmcode/tree/main/llmcode/queries)
from these
