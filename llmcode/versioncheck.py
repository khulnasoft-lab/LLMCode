...
VERSION_CHECK_FNAME = Path.home() / ".llmcode" / "caches" / "versioncheck"


def install_from_main_branch(io):
    """
    Install the latest version of llmcode from the main branch of the GitHub repository.
    """
...
def install_upgrade(io, latest_version=None):
    """
    Install the latest version of llmcode from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer llmcode version v{latest_version} is available."
    else:
        new_ver_text = "Install latest version of llmcode?"
