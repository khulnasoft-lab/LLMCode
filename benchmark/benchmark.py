...
def main(
    ...
    if "LLMCODE_DOCKER" not in os.environ:
...
def load_results(dirname, stats_languages=None):
    ...
    if stats_languages:
        ...
        glob_patterns = [f"{lang}/exercises/practice/*/.llmcode.results.json" for lang in languages]
    else:
        glob_patterns = ["*/exercises/practice/*/.llmcode.results.json"]
...
def summarize_results(dirname, stats_languages=None):
    ...
    if variants["model"]:
        ...
        command = f"llmcode --model {a_model}"
    ...
    for hsh in commit_hashes:
        ...
        try:
            version = subprocess.check_output(
                ["git", "show", f"{hsh}:llmcode/__init__.py"], universal_newlines=True
            )
...
def run_unit_tests(original_dname, testdir, history_fname, test_files):
    ...
    TEST_COMMANDS = {
        ...
        ".go": ["go", "test", "./..."],
        ".js": ["/llmcode/benchmark/npm-test.sh"],
        ".cpp": ["/llmcode/benchmark/cpp-test.sh"],
        ".java": ["./gradlew", "test"],
    }
